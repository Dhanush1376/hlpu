/**
 * mobile-settings.js (Alumni)
 * Mobile-first logic for Alumni Settings page.
 * Reuses /api/settings/* endpoints.
 * Matches Student Settings mobile design exactly.
 */
...
window.MobileSettings = (function () {
    ...
    async function run() {
        ...
        root.innerHTML = `
            ${Nav.renderHeader({ userInitial: getInitials() })}
                ...
                <form id="mob-profile-form">
                    ...
                    <div class="mob-s-form-group">
                        ...
                        <select class="mob-s-form-select" id="mob-gradYear">
                            ${[2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024, 2025, 2026, 2027].map(y => `<option ${(d.graduationYear || '2024') == y ? 'selected' : ''}>${y}</option>`).join('')}
                        </select>
                    </div>
