/**
 * mobile-profile.js (Alumni)
 * Mobile-first logic for Alumni Profile page.
 * Reuses the same /api/profile/me data pipeline.
 * Matches Student Profile mobile design exactly.
 */
...
window.MobileProfile = (function () {
    ...
    async function run() {
        ...
        // Show loading
        root.innerHTML = `
            ${Nav.renderHeader({ userInitial: 'U' })}
    ...
    function getBadgeIcon(badge) {
        ...
        if (b.includes('alumni') || b.includes('mentor')) return 'fas fa-user-tie';
    ...
    function renderProfile(root) {
        ...
        root.innerHTML = `
            ${Nav.renderHeader({ userInitial: initial })}
            ...
            <div class="mob-profile-card">
                ...
                <div class="mob-profile-title">${d.title || 'Alumni'}</div>
...
if (window.innerWidth <= 768 && document.getElementById('mobile-root')) {
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => MobileProfile.run());
    } else {
        MobileProfile.run();
    }
}
